package com.robot.app.ogre
{
   ...
   public class OgreCmdListener extends BaseBeanController
   {
      ...
      private function onOgreList(param1:SocketEvent) : void
      {
         ...
         var _loc5_:Array = [];
         var _loc6_:int = 0;
         var isNewFormat:Boolean = _loc4_.length >= 72;
         while(_loc6_ < 9)
         {
            _loc2_ = _loc4_.readUnsignedInt();
            _loc7_ = isNewFormat ? uint(_loc4_.readUnsignedInt()) : 0;
            if(_loc2_ == 133)
            {
            ...
            if(Boolean(_loc2_))
            {
               OgreController.add(_loc6_,_loc2_,_loc7_);
               _loc5_.push({
                  "_id":_loc2_,
                  "_index":_loc6_
               });
         ...
         if(_loc5_.length > 0)
         {
            var _loc3_:Object = _loc5_[Math.floor(Math.random() * _loc5_.length)];
