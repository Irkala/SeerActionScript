package com.robot.app.galaxy
{
   ...
   public class Galaxy_1 extends Sprite
   {
      ...
      public function destroy() : void
      {
         if(Boolean(this.mapMC))
         {
            try
            {
               (this.mapMC["nextBtn"] as SimpleButton).removeEventListener(MouseEvent.CLICK,this.onNextBtn);
               (this.mapMC["prevBtn"] as SimpleButton).removeEventListener(MouseEvent.CLICK,this.onPrevBtn);
            }
            catch(e:Error)
            {
            }
         ...
         if(Boolean(this.mc))
         {
            this.mc.removeEventListener(MouseEvent.MOUSE_DOWN,this.onMouseDownMap);
         }
         this.mc = null;
         this.mapMC = null;
         this._mainMc = null;
      ...
      private function initGalaxy() : void
      {
         ...
         {
            this.mc["stones_mc"].visible = false;
         }
         if(TasksManager.getTaskStatus(19) == TasksManager.ALR_ACCEPT)
         {
            this.mc["task_19"].alpha = 1;
         }
         else
         {
            this.mc["task_19"].alpha = 0;
         }
         ...
         {
            this.mc["task_17"].alpha = 0;
         }
         this.mc.scrollRect = this.mapScrollRect;
         this.mc.addEventListener(MouseEvent.MOUSE_DOWN,this.onMouseDownMap);
         num = uint(this.mc.numChildren);
         i = 0;
         while(i < num)
         {
      ...
      {
         this._loader = param1;
         this.mc = new (Utils.getClassFromLoader("galaxy_plant_1",this._loader) as Class)();
