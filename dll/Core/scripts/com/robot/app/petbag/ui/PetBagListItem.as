   ...
   public class PetBagListItem extends Sprite
   {
      ...
      private function onShowComplete(param1:DisplayObject) : void
      {
         ...
         if(Boolean(this._showMc))
         {
            ...
            {
               var _shinyArr:Array = ShinyXMLInfo.getShinyArray(this._info.id,this._info.shiny);
               var _colorFilter:ColorMatrixFilter = new ColorMatrixFilter(_shinyArr);
               var _glowArr:Array = ShinyXMLInfo.getGlowArray(this._info.id,this._info.shiny);
               var _glowFilter:GlowFilter = new GlowFilter(uint(_glowArr[0]),int(_glowArr[1]),int(_glowArr[2]),int(_glowArr[3]),Number(_glowArr[4]));
               this._showMc.filters = [this.filte,_glowFilter,_colorFilter];
            }
