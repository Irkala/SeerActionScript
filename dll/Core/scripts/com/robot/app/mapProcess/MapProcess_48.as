package com.robot.app.mapProcess
{
   ...
   public class MapProcess_48 extends BaseMapProcess
   {
      ...
      private var closeArr:Array = [115,121,120,116,95];
      ...
      override protected function init() : void
      {
         ...
         if(conLevel["boss_mc"])
         {
            conLevel["boss_mc"].visible = true;
            this.bindBossClick(conLevel["boss_mc"] as InteractiveObject);
         }
         if(depthLevel["machine"])
         {
            depthLevel["machine"].visible = false;
         }
         this.hideRescuePets();
      }
      ...
      private function bindBossClick(target:InteractiveObject) : void
      {
         if(target == null)
         {
            return;
         ...
         {
            target.addEventListener(MouseEvent.CLICK,this.onBossClick);
         }
      }
      ...
      private function hideRescuePets() : void
      {
         var targetClasses:Array = ["_48_fla.ronghe_5_74","_48_fla.ronghe_6_94"];
         var containers:Array = [conLevel,depthLevel,animatorLevel,topLevel];
         for each(var container in containers)
         {
            if(container != null)
            {
               this.hideTargetsIn(container,targetClasses);
            }
         }
      }
      
      private function hideTargetsIn(container:DisplayObjectContainer, targetClasses:Array) : void
      {
         var i:int = 0;
         ...
         while(i >= 0)
         {
            ...
            clsName = getQualifiedClassName(child);
            if(targetClasses.indexOf(clsName) != -1)
            {
               child.visible = false;
            }
            if(child is DisplayObjectContainer)
            {
               this.hideTargetsIn(child as DisplayObjectContainer,targetClasses);
            }
            i--;
         }
      }
