package com.robot.app.sptGalaxy
{
   ...
   public class BaiHuController
   {
      ...
      private static var _spt:Array = [317,318,320,321,322];
      ...
      public function BaiHuController()
      {
         super();
      }
         ...
         if(event.mapModel.id == _map[_mapIndex])
         {
            if(SystemTimerManager.sysDate.getMinutes() >= 40 && SystemTimerManager.sysDate.getMinutes() < 50)
            {
               ResourceManager.getResource(ClientConfig.getAppRes("GuardBeastPortal"),function(param1:DisplayObject):void
               {
                  ...
                  ToolTipManager.add(_door,"白虎空间传送阵");
      ...
      public static function check(param1:Boolean = false) : void
      {
         ...
         if(b || Boolean(isReady()))
         {
            NpcDialog.show(NPC.KONGJIANSHIZHE,["我是空间使者，白虎空间是罗格星系最神秘的地方，你已具备了进入空间的实力，我现在可以让你传送过去。"],["我决定进去一探究竟。","我还是下次再来吧。"],[function():void
            {
               if(Boolean(_door))
               {
         ...
         {
            NpcDialog.show(NPC.KONGJIANSHIZHE,["我是空间使者，白虎空间是罗格星系最神秘的地方，你只有战胜了0xff0000劳克蒙德，克拉尼特，墨杜萨，肯佩德0xffffff和0xff0000亚伦斯0xffffff，我才会让你进去。"],["我还是下次再来吧。"]);
         }
      }
      
      private static function enter() : void
      {
         MapProcess_483.qingLongStatus = 0;
         MapManager.changeMap(483);
      }
