package com.robot.core.controller
{
   ...
   public class MapController
   {
      ...
      private function initMapFunction(param1:Boolean = true) : void
      {
         ...
         if(MainManager.actorInfo.spiritID != 0)
         {
            pinfo = new PetShowInfo();
            pinfo.catchTime = MainManager.actorInfo.spiritTime;
            pinfo.petID = MainManager.actorInfo.spiritID;
            pinfo.userID = MainManager.actorID;
            pinfo.dv = MainManager.actorInfo.petDV;
            pinfo.shiny = MainManager.actorInfo.petShiny;
            pinfo.skinID = MainManager.actorInfo.petSkin;
            MainManager.actorModel.showPet(pinfo);
         }
         else if(Boolean(PetManager.showInfo))
         {
            pinfo = new PetShowInfo();
