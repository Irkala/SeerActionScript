package com.robot.core.mode
{
   ...
   public class FlyPetModel extends PetModel
   {
      ...
      override public function show(param1:PetShowInfo) : void
      {
         ...
         if(!this._flyPetMc)
         {
            ResourceManager.getResource(ClientConfig.getFlyPetSwfPath(this._petId),this.onFlyPetSwfLoaded,"pet");
         }
         else
         {
            this.showLight();
      ...
      private function onFlyPetSwfLoaded(param1:DisplayObject) : void
      {
         if(this._people == null)
         {
            return;
         ...
         {
            var _shinyArr:Array = ShinyXMLInfo.getShinyArray(this._info.petID,this._info.shiny);
            var _colorFilter:ColorMatrixFilter = new ColorMatrixFilter(_shinyArr);
            var _glowArr:Array = ShinyXMLInfo.getGlowArray(this._info.petID,this._info.shiny);
            var _glowFilter:GlowFilter = new GlowFilter(uint(_glowArr[0]),int(_glowArr[1]),int(_glowArr[2]),int(_glowArr[3]),Number(_glowArr[4]));
            this._flyPetMc.filters = [filte,_glowFilter,_colorFilter];
         }
      ...
      override public function destroy() : void
      {
         ...
         if(this._isMyself)
         {
            var _temp_1:* = (this._people as ActorModel).footMC;
            (this._people as ActorModel).footMC.scaleY = 1;
            _temp_1.scaleX = 1;
         }
         this.hide();
         removeBright();
         ...
         if(this._petId != 0)
         {
            ResourceManager.cancel(ClientConfig.getFlyPetSwfPath(this._petId),this.onFlyPetSwfLoaded);
         }
         if(Boolean(this._flyPetMc))
         {
