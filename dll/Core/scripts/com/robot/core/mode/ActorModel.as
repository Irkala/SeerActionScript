package com.robot.core.mode
{
   ...
   public class ActorModel extends BasePeoleModel
   {
      ...
      public function moveAndAction(param1:Point, param2:Function) : void
      {
         ...
         var onWalkEnd:Function = function(param3:Event):void
         {
            if(executed)
            {
               return;
            ...
            executed = true;
            removeEventListener(RobotEvent.WALK_END,onWalkEnd);
            removeEventListener(Event.ENTER_FRAME,onEnterFrameCheck);
            if(Boolean(callback))
            {
               callback();
            }
         };
         var onEnterFrameCheck:Function = function(param3:Event):void
         {
            removeEventListener(Event.ENTER_FRAME,onEnterFrameCheck);
            if(executed)
            {
            ...
            if(Boolean(walk) && !walk.isPlaying)
            {
               executed = true;
               removeEventListener(RobotEvent.WALK_END,onWalkEnd);
               if(Boolean(callback))
         ...
         addEventListener(RobotEvent.WALK_END,onWalkEnd);
         addEventListener(Event.ENTER_FRAME,onEnterFrameCheck);
         walkAction(point);
      }
   }
}
