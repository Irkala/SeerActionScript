package com.robot.core.mode
{
   ...
   public class PetModel extends ActionSpriteModel
   {
      ...
      public function show(param1:PetShowInfo) : void
      {
         ...
         ResourceManager.getResource(ClientConfig.getPetSwfPath(this.info.skinID != 0 ? uint(this.info.skinID) : uint(this.info.petID)),this.onPetSwfLoaded,"pet");
      }
      ...
      private function onPetSwfLoaded(param1:DisplayObject) : void
      {
         if(this._people == null)
         {
            return;
         ...
         if(this.info.shiny != 0)
         {
            var _shinyArr:Array = ShinyXMLInfo.getShinyArray(this.info.petID,this.info.shiny);
            var _colorFilter:ColorMatrixFilter = new ColorMatrixFilter(_shinyArr);
            var _glowArr:Array = ShinyXMLInfo.getGlowArray(this.info.petID,this.info.shiny);
            var _glowFilter:GlowFilter = new GlowFilter(uint(_glowArr[0]),int(_glowArr[1]),int(_glowArr[2]),int(_glowArr[3]),Number(_glowArr[4]));
            this._petMc.filters = [this.filte,_glowFilter,_colorFilter];
         }
         this.bright();
         if(_walk.isPlaying)
         {
